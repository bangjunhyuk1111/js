...
app.get('/products', async (req, res) => {
    const { category } = req.query //카테고리와 일치하는것을 찾음, 없으면 모든상품
    if(category){
        const products = await Product.find({category})
        res.render('products/index' , {products, category} ) //페이지 제목용
    }else{
        const products = await Product.find({}) 
        res.render('products/index' , {products, category: 'All'} ) //페이지 제목용
}
...
app.delete('/products/:id', async (req, res) => {
    const { id } = req.params;
    const deldteProduct = await Product.findByIdAndDelete(id)
